...
public class BroadcastGen {
  ...
  public BroadcastGen(TransactionConfig config) {
  ...
  public void broadcastTransactions() {
      ...
      saveTransactionIDPool.submit(() -> {
        ...
        try {
          bufferedWriter = new BufferedWriter(
              new FileWriter("gen-transactionsID" + value + ".csv"));
    ...
    for (int index = 0; index <= dispatchCount; index++) {
      ...
      try {
        isFinishSend = false;
        File f = new File("gen-transaction" + index + ".csv");
...
//          log.info(i++ + "   " + transaction.toString());
        ...
        try {
          log.info("Finished process dispatch task {}", index);
          fis.close();
        } catch (IOException e) {
          e.printStackTrace();
        }
